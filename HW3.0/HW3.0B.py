...
# -*- coding: utf-8 -*-
"""
Created on Wed Oct  5 16:07:31 2021
...
if(model_type == "ANN"):
    for i in range(1, len(layers)):
        NFIT += layers[i-1] * layers[i] + layers[i]
...
def model(x, p):
    if(model_type=="linear" or model_type=="logistic" ):
        ...
        return  out
    if(model_type == "ANN"):
        WBs = extract_submatrices(p)
        return ANN_model(x, WBs)
        
#WBs means weights and bias at each layer
def ANN_model(x, WBs):
    x_trans = np.transpose(x)
    #how many times we need to apply activation function
    num_of_act = (int)(len(WBs) / 2 - 1)
    for i in range(num_of_act):
        #weight index
        w_idx = 2*i
        #bias index
        b_idx = 2*i+1
        layer_res = np.matmul(WBs[w_idx], x_trans) + WBs[b_idx]
        #Apply sigmoid / Tanh activation function
        act_res = 0
        if act == "SIG":
            act_res = S(layer_res)
        if act == "TANH":
            act_res = (np.exp(layer_res)-np.exp(-layer_res)) / (np.exp(layer_res)+np.exp(-layer_res))
        #treat the output from current layer as the input to next layer
        x_trans = act_res
    #Don't apply activation function on output layer
    res = np.matmul(WBs[-2], x_trans) + WBs[-1]
...
def loss(p,index_2_use):
	...
	training_loss = np.mean(errors**2.0)	 + GAMMA_L1*sum(np.abs(p)) + GAMMA_L2*sum(p**2)
